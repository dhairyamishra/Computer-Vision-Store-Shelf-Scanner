...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; } body { margin: 0; min-height: 100vh; background: radial-gradient(circle at 15% 0%, #1d3b63 0, transparent 32rem), #0b1020; }
    ...
    .preview { display: none; width: 100%; max-height: 260px; object-fit: contain; border-radius: 12px; margin-top: 16px; background: #060a13; } .preview.show { display: block; } video.preview.portrait { width: min(100%, 360px); max-height: 520px; margin-left: auto; margin-right: auto; }
...
<body>
  <main>
    ...
    <section class="card" aria-label="Create an audit">
      <form id="audit-form">
        ...
        <video id="video-preview" class="preview" controls preload="metadata" aria-label="Selected shelf video preview"></video>
  ...
  <script>
    const form = document.querySelector('#audit-form'); const account = document.querySelector('#account'); const media = document.querySelector('#media'); const dropZone = document.querySelector('#drop-zone'); const browse = document.querySelector('#browse'); const status = document.querySelector('#status'); const processing = document.querySelector('#processing'); const submit = document.querySelector('#submit'); const result = document.querySelector('#result'); const output = document.querySelector('#output'); const imagePreview = document.querySelector('#image-preview'); const videoPreview = document.querySelector('#video-preview'); let selectedFile; let previewUrl;
    ...
    function setMedia(file, source) { selectedFile = file; if (previewUrl) URL.revokeObjectURL(previewUrl); imagePreview.removeAttribute('src'); imagePreview.classList.remove('show'); videoPreview.pause(); videoPreview.removeAttribute('src'); videoPreview.load(); videoPreview.classList.remove('show'); videoPreview.classList.remove('portrait'); previewUrl = file ? URL.createObjectURL(file) : undefined; if (file && file.type.startsWith('image/')) { imagePreview.src = previewUrl; imagePreview.classList.add('show'); } else if (file && file.type.startsWith('video/')) { videoPreview.src = previewUrl; videoPreview.classList.add('show'); } status.textContent = file ? source + ': ' + file.name : ''; }
    videoPreview.addEventListener('loadedmetadata', () => { videoPreview.classList.toggle('portrait', videoPreview.videoHeight > videoPreview.videoWidth); });
